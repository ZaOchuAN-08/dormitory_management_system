...
def login():
    ...
    try:
        ...
        if not user_id or len(user_id)!=9 or not password:
            return jsonify({'success': False, 'message': '请输入有效ID或密码'}), 400
...
def get_student_info():
    ...
    try:
        ...
        if info:
            app.logger.info(f"成功获取学生 {student_id} 信息")
            # print(info)
            return jsonify({'success': True, 'info': info}), 200
        else:
            return jsonify({'success': False, 'message': '未找到该学生信息'}), 404

    except Exception as e:
        app.logger.error(f"查询学生信息时发生错误: {e}")
        return jsonify({'success': False, 'message': '服务器查询错误'}), 500
    finally:
        if cur:
            cur.close()
...
def update_student_phone():
    ...
    if not student_id or not new_phone:
        return jsonify({'success': False, 'message': '缺少ID或新手机号码'}), 400
    ...
    try:
        ...
        if cur.rowcount > 0:
            ...
            app.logger.info(f"学生 {student_id} 手机号更新成功")
            return jsonify({'success': True, 'message': '手机号码更新成功！'}), 200
        else:
            # 即使 ID 存在但 phone 没变，rowcount 也可能为 0，但更可能是 ID 不存在
            return jsonify({'success': False, 'message': '手机号未改变'}), 404

    except Exception as e:
        mysql.connection.rollback()
        app.logger.error(f"更新学生手机号时发生错误: {e}")
        return jsonify({'success': False, 'message': '服务器更新错误'}), 500
    finally:
        if cur:
            cur.close()
...
def get_tutor_info():
    ...
    if not tutor_id:
        return jsonify({'success': False, 'message': '缺少导师ID参数'}), 400
    ...
    try:
        ...
        if info:
            app.logger.info(f"成功获取导师 {tutor_id} 信息")
            # print(info)
            return jsonify({'success': True, 'info': info}), 200
        else:
            return jsonify({'success': False, 'message': '未找到该导师信息'}), 404
    except Exception as e:
        app.logger.error(f"查询导师信息时发生错误: {e}")
        return jsonify({'success': False, 'message': '服务器查询错误'}), 500
    finally:
        if cur:
            cur.close()
...
def update_tutor_phone():
    ...
    try:
        ...
        if cur.rowcount > 0:
            mysql.connection.commit()
            app.logger.info(f"导师 {tutor_id} 手机号更新成功")
            return jsonify({'success': True, 'message': '手机号码更新成功！'}), 200
        else:
            # 即使 ID 存在但 phone 没变，rowcount 也可能为 0，但更可能是 ID 不存在
            return jsonify({'success': False, 'message': '手机号未改变'}), 404
        ...
        mysql.connection.rollback()
        app.logger.error(f"更新导师手机号时发生错误: {e}")
        return jsonify({'success': False, 'message': '服务器更新错误'}), 500
    finally:
        if cur:
            cur.close()
...
def get_warden_info():
    ...
    if not warden_id:
        return jsonify({'success': False, 'message': '缺少舍监ID参数'}), 400
    ...
    try:
        ...
        if info:
            app.logger.info(f"成功获取舍监 {warden_id} 信息")
            # print(info)
            return jsonify({'success': True, 'info': info}), 200
        else:
            return jsonify({'success': False, 'message': '未找到该舍监信息'}), 404

    except Exception as e:
        app.logger.error(f"查询舍监信息时发生错误: {e}")
        return jsonify({'success': False, 'message': '服务器查询错误'}), 500
    finally:
        if cur:
            cur.close()
...
def update_warden_phone():
    ...
    try:
        ...
        if cur.rowcount > 0:
            mysql.connection.commit()
            app.logger.info(f"舍监 {warden_id} 手机号更新成功")
            return jsonify({'success': True, 'message': '手机号码更新成功！'}), 200
        else:
            # 即使 ID 存在但 phone 没变，rowcount 也可能为 0，但更可能是 ID 不存在
            return jsonify({'success': False, 'message': '手机号未改变'}), 404
        ...
        mysql.connection.rollback()
        app.logger.error(f"更新舍监手机号时发生错误: {e}")
        return jsonify({'success': False, 'message': '服务器更新错误'}), 500
    finally:
        if cur:
            cur.close()
...
def get_student_dorm_info():
    ...
    try:
        ...
        if dorm_info:
            app.logger.info(f"成功获取学生 {student_id} 宿舍/电费信息")
            #print(dorm_info)
            return jsonify({'success': True, 'dorm_info': dorm_info}), 200
        else:
            return jsonify({'success': False, 'message': '未找到宿舍信息'}), 404

    except Exception as e:
        app.logger.error(f"Error fetching dorm info: {str(e)}")
        return jsonify({'success': False, 'message': '服务器错误，无法查询宿舍信息'}), 500
    finally:
        if cur:
            cur.close()
...
def recharge_electricity():
    ...
    try:
        ...
        if not student_id or not amount or not isinstance(amount, (int, float)) or amount <= 0:
            return jsonify({'success': False, 'message': '无效的充值请求'}), 400
        cur = mysql.connection.cursor()
        ...
        if not result or not result['ROOM_ID']:
            cur.close()
            return jsonify({'success': False, 'message': '学生未分配宿舍，无法充值。'}), 404
        ...
        return jsonify({'success': True, 'message': f'成功为房间 {room_id} 充值 {amount:.2f} 元。', 'new_balance': '需要重新查询'}), 200

    except Exception as e:
        app.logger.error(f"Error processing recharge: {str(e)}")
        mysql.connection.rollback() # 失败时回滚
        return jsonify({'success': False, 'message': '服务器错误，充值失败。'}), 500
    finally:
        if cur: cur.close()
...
def get_tutor_students():
    ...
    try:
        tutor_id = request.args.get('id')
        if not tutor_id:
            return jsonify({'success': False, 'message': '缺少舍监ID'}), 400
        ...
        if floor_info:
            app.logger.info(f"成功获取导师 {tutor_id} 管理的楼层信息")
        else:
            return jsonify({'success': False, 'message': '未找到管理的楼栋信息'}), 404
        ...
        if students:
            app.logger.info(f"成功获取导师 {tutor_id} 管理的楼层学生信息")
        ...
        return jsonify({'success': False, 'message': '服务器错误，无法查询学生信息'}), 500
    finally:
        if cur: cur.close()
...
def get_warden_tutors():
    ...
    try:
        ...
        if building_info:
            app.logger.info(f"成功获取舍监 {warden_id} 管理的楼层信息")
        else:
            return jsonify({'success': False, 'message': '未找到管理的楼栋信息'}), 404
        ...
        tutors = cur.fetchall()
        if tutors:
            app.logger.info(f"成功获取舍监 {warden_id} 管理的楼层导师信息")
        ...
        app.logger.error(f"Error fetching warden tutors: {str(e)}")
        return jsonify({'success': False, 'message': '服务器错误，无法查询导师信息'}), 500
    finally:
        if cur: cur.close()
...
def get_warden_dorm_status():
    ...
    try:
        ...
        return jsonify({'success': False, 'message': '服务器错误，无法查询宿舍状态信息'}), 500
    finally:
        if cur: cur.close()
...
def get_student_rooms():
    ...
    try:
        ...
        if room:
            ...
            return jsonify({'success': False, 'rooms': [], 'message': '未找到宿舍信息'}), 404
    finally:
        cur.close()
...
def get_student_buildings():
    student_id = request.args.get('id')
    if not student_id:
        return jsonify({'success': False, 'message': '缺少学生楼栋'}), 400
    cur = mysql.connection.cursor()
    try:
        ...
        if building:
            ...
            return jsonify({'success': False, 'buildings': [], 'message': '未找到楼栋信息'}), 404
    finally:
        cur.close()
...
def get_adjust_rooms():
    ...
    if not building_id or not floor_id:
        return jsonify({'success': False, 'message': '缺少楼栋或楼层信息'}), 400
    ...
    try:
        ...
        if rooms:
            ...
            return jsonify({'success': False, 'rooms': [], 'message': '无可用房间'})
    finally:
        if cur:
            cur.close()
...
def get_adjust_beds():
    ...
    if not room_id:
        return jsonify({'success': False, 'message': '缺少房间信息'}), 400
    ...
    try:
        ...
        if beds:
            ...
            return jsonify({'success': False, 'beds': [], 'message': '无可用床位'})
    finally:
        if cur:
            cur.close()
...
def submit_repair_request():
    ...
    if not student_id or not room_id or not repair_type:
        return jsonify({'success': False, 'message': '缺少信息'}), 400
    cur = mysql.connection.cursor()
    ...
    try:
        ...
        if not tutor_info:
            return jsonify({'success': False, 'message': '未找到负责该楼层的导师'}), 404
        ...
        mysql.connection.commit()
        return jsonify({'success': True, 'message': '维修申请已提交！导师会尽快处理。'})
    except Exception as e:
        app.logger.error(f"Error: 处理申请出现错误")
        return jsonify({'success': False, 'message': '服务器错误，无法处理信息'}), 500
    finally:
        if cur:
            cur.close()
...
def get_tutor_pending_requests():
    tutor_id = request.args.get('id')
    if not tutor_id:
        return jsonify({'success': False, 'message': '缺少导师ID'}), 400
...
def process_repair_request():
    ...
    try:
        ...
        mysql.connection.commit()
        return jsonify({'success': True, 'message': f'已{action}该申请'})
    except Exception as e:
        app.logger.error(f"Error: 处理申请出现错误")
        return jsonify({'success': False, 'message': '服务器错误，无法处理信息'}), 500
    finally:
...
def submit_adjust_request():
    ...
    try:
        ...
        if not warden_info:
            return jsonify({'success': False, 'message': '未找到该楼栋舍监'}), 404
        ...
        mysql.connection.commit()
        return jsonify({'success': True, 'message': '宿舍调整申请已提交，等待舍监处理'})
    except Exception as e:
...
def process_adjust_request():
    ...
    try:
        ...
        req = cur.fetchone()
        if not req:
            return jsonify({'success': False, 'message': '未找到申请'}), 404
        ...
        mysql.connection.commit()
        return jsonify({'success': True, 'message': f'已{action}该宿舍调整申请'})
    except Exception as e:
        app.logger.error(f"Error: {e}")
        return jsonify({'success': False, 'message': '服务器错误'}), 500
    finally:
...
if __name__ == '__main__':
    # 打印明确的启动信息
    print("--------------------------------------------------")
    print(f"Flask App 正在启动...")
    print(f"数据库: {app.config['MYSQL_DB']} @ {app.config['MYSQL_HOST']}")
    print(f"前端请访问: http://127.0.0.1:5000/")
    print("--------------------------------------------------")
