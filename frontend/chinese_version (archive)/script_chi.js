...
function showDashboard(role, userInfo) {
    ...
    if (dashboard) {
        ...
        loginMessage.textContent = '登录成功，但角色信息未知。';
        logout(); // 角色未知则退出登录
    }
}
...
function setupDashboardNavigation(dashboard, defaultTabId) {
    ...
    navButtons.forEach(button => {
        button.addEventListener('click', (event) => {
            ...
            else if (targetId === 'tutor-info-tab' && role === 'tutor') {loadTutorInfo(userId);}
            else if (targetId === 'warden-info-tab' && role === 'warden') {loadWardenInfo(userId);}

            else if (targetId === 'student-dorm-tab' && role === 'student') {loadStudentDormInfo(userId);}
            else if (targetId === 'student-repair-tab' && role === 'student') {loadStudentRoom(userId);}
            else if (targetId === 'student-adjust-tab' && role === 'student') {loadStudentAdjust(userId);}
            else if (targetId === 'tutor-student-tab' && role === 'tutor') {loadTutorStudentInfo(userId);}
            else if (targetId === 'tutor-repair-tab' && role === 'tutor') {loadTutorRepairRequests(userId);}
            else if (targetId === 'warden-tutor-tab' && role === 'warden') {loadWardenTutorInfo(userId);}
            else if (targetId === 'warden-student-tab' && role === 'warden') {loadWardenStudentInfo(userId);}
            else if (targetId === 'warden-dorm-tab' && role === 'warden') {loadWardenDormInfo(userId);}
            else if (targetId === 'warden-adjust-tab' && role === 'warden') {loadWardenAdjustRequests(userId);}
        });
    });
}
...
async function handleLogin(event) {
    ...
    try {
        ...
        if (response.ok && data.success) {
            ...
            const message = data.message || '登录失败，请检查账号和密码。';
            loginMessage.textContent = message;
        }
        
    } catch (error) {
        console.error('网络或服务器连接错误:', error);
        loginMessage.textContent = '无法连接到服务器，请检查后端是否运行在 ' + API_URL;
    } finally {
...
async function loadStudentInfo(studentId) {
    ...
    infoStudentMessage.textContent = '正在加载个人信息...';
    ...
    try {
        ...
        if (response.ok && data.success) {
            ...
            document.getElementById('info-phone').textContent = info.STUDENT_PHONE_NUM || '未设置';
            document.getElementById('info-email').textContent = info.STUDENT_EMAIL_ADDRESS || 'N/A';
            infoStudentMessage.textContent = '信息加载完成。';
            infoStudentMessage.style.color = 'green';
        } else {
            infoStudentMessage.textContent = data.message || '加载信息失败。';
            infoStudentMessage.style.color = 'red';
        }
    } catch (error) {
        console.error('加载学生信息时发生网络错误或JSON解析错误:', error);
        infoStudentMessage.textContent = '网络错误，无法获取学生信息。';
        infoStudentMessage.style.color = 'red';
    }
}
...
async function loadStudentDormInfo(studentId) {
    ...
    dormMessage.textContent = '正在加载宿舍信息...';
    ...
    try {
        ...
        if (res.ok && data.success) {
            ...
            dormMessage.textContent = '宿舍信息加载完成。';
            dormMessage.style.color = 'green';
        } else {
            dormMessage.textContent = data.message || '未能获取宿舍信息。';
            dormMessage.style.color = 'red';
            balanceEl.textContent = '0.00';
        }
    } catch (err) {
        console.error('获取宿舍信息出错：', err);
        dormMessage.textContent = '网络错误，无法获取宿舍信息。';
        dormMessage.style.color = 'red';
        balanceEl.textContent = '0.00';
    }
}
...
async function handleRecharge(event) {
    ...
    if (!userId) {
        rechargeMsg.textContent = '未检测到用户，请重新登录。';
        rechargeMsg.style.color = 'red';
        return;
    }
    if (!amount || isNaN(amount) || amount <= 0) {
        rechargeMsg.textContent = '请输入有效的充值金额（大于10）。';
        rechargeMsg.style.color = 'red';
        return;
    }
    ...
    rechargeMsg.textContent = '正在提交充值请求...';
    ...
    try {
        ...
        if (res.ok && data.success) {
            ...
            rechargeMsg.textContent = data.message || `充值成功：${amount.toFixed(2)} 元`;
            rechargeMsg.style.color = 'green';
            amountInput.value = ''; // 清空输入框
        } else {
            rechargeMsg.textContent = data.message || '充值失败，请稍后重试。';
            rechargeMsg.style.color = 'red';
        }
    } catch (err) {
        console.error('充值请求出错：', err);
        rechargeMsg.textContent = '网络错误，充值失败。';
        rechargeMsg.style.color = 'red';
    } finally {
        rechargeBtn.disabled = false;
        rechargeBtn.textContent = '立即充值';
    }
}

async function loadStudentRoom(studentId) {
    const selectEl = document.getElementById('repair-room');
    selectEl.innerHTML = '<option value="" disabled selected>加载中...</option>';
    ...
    try {
        ...
        if (response.ok && data.success && data.rooms.length > 0) {
            ...
            selectEl.innerHTML = '<option value="" disabled>未找到宿舍号</option>';
        }
    } catch (error) {
        console.error('加载学生宿舍号失败:', error);
        selectEl.innerHTML = '<option value="" disabled>加载失败</option>';
    }
}
...
async function loadStudentAdjust(studentId) {
    ...
    buildingEl.innerHTML = '<option value="" disabled selected>加载中...</option>';
    floorEl.innerHTML = '<option value="" disabled selected>请选择楼层</option>';
    roomEl.innerHTML = '<option value="" disabled selected>请选择房间号</option>';
    bedEl.innerHTML = '<option value="" disabled selected>请选择床号</option>';

    try {
        // 获取学生信息（含性别）
        const studentRes = await fetch(`${API_URL}/student/info?id=${studentId}`);
        const studentData = await studentRes.json();
        if (!studentData.success) throw '未获取学生信息';
        ...
        if (buildingRes.ok && buildingData.success && buildingData.buildings.length > 0){
            ...
            buildingEl.innerHTML = '<option value="" disabled>未找到楼栋</option>';
        }
        ...
        floorEl.addEventListener('change', async () => {
            roomEl.innerHTML = '<option value="" disabled selected>加载中...</option>';
            bedEl.innerHTML = '<option value="" disabled selected>请选择床号</option>';
            ...
            if (data.success && data.rooms.length > 0) {
                ...
                roomEl.innerHTML = '<option value="" disabled>无可用房间</option>';
            }
        });
        ...
        roomEl.addEventListener('change', async () => {
            ...
            bedEl.innerHTML = '<option value="" disabled selected>加载中...</option>';
            ...
            if (data.success && data.beds.length > 0) {
                ...
                bedEl.innerHTML = '<option value="" disabled>无可用床位</option>';
            }
        });

    } catch (err) {
        console.error('加载宿舍调整表单失败', err);
        buildingEl.innerHTML = '<option value="" disabled>加载失败</option>';
    }
}
...
async function loadTutorInfo(tutorId) {
    ...
    infoTutorMessage.textContent = '正在加载个人信息...';
    ...
    try {
        ...
        if (response.ok && data.success) {
            ...
            document.getElementById('info-tutor-phone').textContent = info.TUTOR_PHONE_NUM || '未设置';
            infoTutorMessage.textContent = '信息加载完成。';
            infoTutorMessage.style.color = 'green';
        } else {
            infoTutorMessage.textContent = data.message || '加载信息失败。';
            infoTutorMessage.style.color = 'red';
        }
    } catch (error) {
        console.error('加载导师信息时发生网络错误或JSON解析错误:', error);
        infoTutorMessage.textContent = '网络错误，无法获取导师信息。';
        infoTutorMessage.style.color = 'red';
    }
}
...
async function loadTutorStudentInfo(tutorId) {
    ...
    messageTS.textContent = '正在加载学生信息...';
    ...
    try {
        ...
        if (response.ok && data.success) {
            messageTS.textContent = `楼层 ${data.floor_id} 学生信息加载成功。`;
            ...
            if (data.students && data.students.length > 0) {
                ...
                listEl.innerHTML = '<tr><td colspan="8" class="text-center py-4">未找到该楼栋的学生。</td></tr>';
            }
        } else {
            messageTS.textContent = data.message || '获取学生信息失败。';
            messageTS.style.color = 'red';
        }
    } catch (error) {
        console.error('获取学生信息时发生网络错误:', error);
        messageTS.textContent = '网络错误，无法加载学生信息。';
        messageTS.style.color = 'red';
    }
}
        ...
        if (res.ok && data.success && data.students.length > 0) {
            ...
            data.students.forEach(student => {
                ...
                tr.innerHTML = `
                    ...
                    <td>
                        <button class="btn-action btn-approve" data-request="${student.REQUEST_ID}" data-room="${student.ROOM_ID}" data-action="通过">通过</button>
                        <button class="btn-action btn-reject" data-request="${student.REQUEST_ID}" data-room="${student.ROOM_ID}" data-action="拒绝">拒绝</button>
                    </td>
                `;
            ...
            tbody.innerHTML = '<tr><td colspan="5" class="req-empty-table text-center">暂无待处理申请</td></tr>';
        }
    } catch (err) {
        console.error('加载维修请求失败:', err);
        tbody.innerHTML = '<tr><td colspan="4" class="req-empty-table text-center">加载失败</td></tr>';
    }
}
...
async function loadWardenInfo(wardenId) {
    ...
    infoWardenMessage.textContent = '正在加载个人信息...';
    ...
    try {
        ...
        if (response.ok && data.success) {
            ...
            document.getElementById('info-warden-phone').textContent = info.WARDEN_PHONE_NUM || '未设置';
            infoWardenMessage.textContent = '信息加载完成。';
            infoWardenMessage.style.color = 'green';
        } else {
            infoWardenMessage.textContent = data.message || '加载信息失败。';
            infoWardenMessage.style.color = 'red';
        }
    } catch (error) {
        console.error('加载舍监信息时发生网络错误或JSON解析错误:', error);
        infoWardenMessage.textContent = '网络错误，无法获取舍监信息。';
        infoWardenMessage.style.color = 'red';
    }
}
...
async function loadWardenTutorInfo(wardenId) {
    ...
    messageWT.textContent = '正在加载导师信息...';
    ...
    try {
        ...
        if (response.ok && data.success) {
            messageWT.textContent = `楼栋 ${data.building_id} 导师信息加载成功。`;
            messageWT.style.color = 'green';
            if (data.tutors && data.tutors.length > 0) {
                data.tutors.forEach(tutor => {
                    // 修正：使用新的字段名 TUTOR_ID, TUTOR_NAME, TUTOR_PHONE_NUM, TUTOR_GENDER, Pending_Count
                    const row = `
                ...
                listEl.innerHTML = '<tr><td colspan="6" class="text-center py-4">未找到负责该楼栋的导师。</td></tr>';
            }
        } else {
            messageWT.textContent = data.message || '获取导师信息失败。';
            messageWT.style.color = 'red';
        }
    } catch (error) {
        console.error('获取导师信息时发生网络错误:', error);
        messageWT.textContent = '网络错误，无法加载导师信息。';
        messageWT.style.color = 'red';
    }
}
...
async function loadWardenStudentInfo(wardenId) {
    ...
    messageWS.textContent = '正在加载学生信息...';
    ...
    try {
        ...
        if (response.ok && data.success) {
            messageWS.textContent = `楼栋 ${data.building_id} 学生信息加载成功。`;
            ...
            messageWS.textContent = data.message || '获取学生信息失败。';
            messageWS.style.color = 'red';
        }
    } catch (error) {
        console.error('获取学生信息时发生网络错误:', error);
        messageWS.textContent = '网络错误，无法加载学生信息。';
        messageWS.style.color = 'red';
    }
}
...
async function loadWardenDormInfo(wardenId) {
    ...
    messageElem.textContent = '正在加载宿舍信息...';
    ...
    try {
        ...
        if (response.ok && data.success) {
            ...
            messageElem.textContent = `楼栋 ${data.building_id} 宿舍信息加载成功。`;
            ...
            if (rooms && rooms.length > 0) {
                ...
                listEl.innerHTML = '<tr><td colspan="6" class="text-center py-4">未找到该楼栋的学生。</td></tr>';
            }
        } else {
            messageElem.textContent = data.message || '获取宿舍信息失败。';
            messageElem.style.color = 'red';
        }
    } catch (error) {
        console.error('获取宿舍信息时发生网络错误:', error);
        messageElem.textContent = '网络错误，无法加载宿舍信息。';
        messageElem.style.color = 'red';
    }
}
        ...
        if (res.ok && data.success && data.requests.length > 0) {
            ...
            data.requests.forEach(req => {
                ...
                tr.innerHTML = `
                    ...
                    <td>
                        <button class="btn-action btn-approve" data-request="${req.REQUEST_ID}" data-action="通过">通过</button>
                        <button class="btn-action btn-reject" data-request="${req.REQUEST_ID}" data-action="拒绝">拒绝</button>
                    </td>
                `;
                tbody.appendChild(tr);
            });
            ...
            tbody.innerHTML = '<tr><td colspan="5" class="text-center">暂无待处理申请</td></tr>';
        }
    } catch (err) {
...
function toggleEditPhone(show) {
    ...
    if (show) {
        if (role === 'student'){
            ...
            input.value = currentStudentPhone === '未设置' ? '' : currentStudentPhone;
        }else if (role === 'tutor'){
            const currentTutorPhone = document.getElementById('info-tutor-phone').textContent;
            input.value = currentTutorPhone === '未设置' ? '' : currentTutorPhone;
        }else if (role === 'warden'){
            const currentWardenPhone = document.getElementById('info-warden-phone').textContent;
            input.value = currentWardenPhone === '未设置' ? '' : currentWardenPhone;
        }
...
async function handleUpdatePhone(event) {
    ...
    if (!newPhone) {
        infoMessage.textContent = '手机号码不能为空。';
    ...
    infoMessage.textContent = '正在提交修改...';
    ...
    try {
        ...
        if (response.ok && data.success) {
            ...
            infoMessage.textContent = data.message || '更新手机号失败。';
            infoMessage.style.color = 'red';
        }

    } catch (error) {
        console.error('更新手机号时发生网络错误:', error);
        infoMessage.textContent = '网络错误，无法连接到服务器。';
        infoMessage.style.color = 'red';
    } finally {
    ...
    if (repairForm) {
        ...
        repairForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                messageEl.textContent = '网络错误，提交失败';
                messageEl.style.color = 'red';
            }
        });
    }
